...
def _target_encoder(smooth: str | float = "auto") -> TargetEncoder:
...
def build_linear_preprocessor(te_smooth: str | float = "auto") -> Pipeline:
    ...
    column_block = ColumnTransformer(
        transformers=[
            ...
            (
                ...
                Pipeline(
                    [
                        ("encode", _target_encoder(smooth=te_smooth)),
                        ("scale", StandardScaler()),
                    ]
                ),
                ENGINEERED_TARGET_ENCODE,
...
def build_tree_preprocessor(te_smooth: str | float = "auto") -> Pipeline:
    ...
    column_block = ColumnTransformer(
        transformers=[
            ...
            (
                "te",
                _target_encoder(smooth=te_smooth),
                ENGINEERED_TARGET_ENCODE,
            ),
        ]
    )
